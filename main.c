...
int main(){
	...
	printf("Conjunto A: ");
	mostrarTData(conjuntoA); // Deberï¿½a mostrar {Azul, Rojo} o similar
	printf("\n\n");
	...
	*/
	...
	// --- LIBERACIï¿½N DE MEMORIA ---
	
	// 1. Liberamos el string original
	liberarTData(texto);
	
	// 2. Liberamos la lista (esta funciï¿½n es recursiva, 
	// limpiarï¿½ todos los eslabones y los caracteres adentro)
	liberarTData(lista);
    ...
    // 1. CreaciÃ³n y carga de la Lista Original
    ...
    // 4. Prueba de ConcatenaciÃ³n (concat)
    printf("\n\n--- Test Concatenacion ---\n");
